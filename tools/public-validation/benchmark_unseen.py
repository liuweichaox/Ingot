#!/usr/bin/env python3
"""Run the protocol-frozen unseen reaction-process acceptance."""
...
PROTOCOL_PATH = ROOT / "unseen-protocol.json"
PRIMARY = "ingot-current-with-preregistered-process-feature"
ABLATION = "ingot-current-without-process-feature"
DATASETS = {
    "fullerenes": {
        "id_field": "setting_id",
        "control_fields": (
...
def load_protocol() -> dict:
    ...
    if protocol.get("schema") != "ingot-unseen-data-acceptance-protocol":
        raise ValueError("unsupported public validation protocol")
    if set(protocol["sources"]) != set(DATASETS):
        raise ValueError("protocol sources and unseen-data adapters disagree")
    expected = {PRIMARY, ABLATION, *protocol["methods"]["baselines"]}
    if len(expected) != 6:
        raise ValueError("unseen acceptance requires two Ingot variants and four baselines")
    unsuccessful = int(protocol["statistics"]["unsuccessful_trial_value"])
    for name, settings in protocol["datasets"].items():
        if name not in DATASETS:
            raise ValueError(f"unsupported unseen-acceptance dataset: {name}")
        if tuple(settings["controls"]) != DATASETS[name]["control_fields"]:
            raise ValueError(f"unseen-acceptance control order changed for {name}")
        if tuple(settings["contexts"]) != DATASETS[name]["context_fields"]:
            raise ValueError(f"unseen-acceptance context order changed for {name}")
        initial = int(settings["initial_observations"])
        additional = int(settings["additional_trial_budget"])
        if initial < 3 or additional < 1:
            raise ValueError(f"invalid unseen-acceptance episode budget for {name}")
        if additional + 1 != unsuccessful:
            raise ValueError("all unseen-acceptance units must share the capped failure value")
        rule = settings["objective"]["threshold_rule"]
        if (
            rule.get("kind") != "empirical-quantile-within-evaluation-unit"
            or rule.get("method") != "linear"
        ):
            raise ValueError("unseen acceptance requires the frozen within-unit quantile rule")
...
def evaluation_fingerprint(protocol: dict) -> str:
    ...
    paths = [
        ROOT / "benchmark_unseen.py",
        ROOT / "prepare_unseen_fixtures.py",
        *[
...
def require_frozen(protocol: dict) -> None:
    ...
    if protocol.get("status") != "frozen" or any(
        ...
        raise RuntimeError(
            "unseen-data protocol is not frozen; commit the candidate, data, adapter, and draft protocol first"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "unseen-data fingerprint does not match the frozen algorithm, adapter, data, dependencies, and protocol"
        )
...
def build_campaign(
    ...
    campaign = Campaign(
        ...
        context={
            ...
            "protocol": "unseen-data-acceptance",
            **context,
        },
    )
    ...
    *,
    ...
    while len(episodes) < episode_count:
        if candidate_index > 1_000_000:
            raise RuntimeError(
                f"unable to generate enough unseen-acceptance episodes for {unit_id}"
            )
...
def paired_effect(records: list[dict], comparator: str, protocol: dict) -> dict:
    ...
    if mechanism:
        relative_floor = float(
            statistics["mechanism_ablation_relative_reduction_ci_lower"]
        )
        unit_floor = float(statistics["mechanism_ablation_unit_margin"])
    elif response_surface:
        relative_floor = float(statistics["response_surface_noninferiority_margin"])
        unit_floor = float(statistics["response_surface_noninferiority_margin"])
    else:
        relative_floor = float(
...
def main() -> None:
    ...
    if args.integrity_only:
        ...
        payload = {
            "schema": "ingot-unseen-data-acceptance-result",
            "protocol_sha256": sha256(PROTOCOL_PATH),
