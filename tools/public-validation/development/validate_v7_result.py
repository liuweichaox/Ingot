#!/usr/bin/env python3
"""Independently recompute the frozen v7 result from episode records."""
...
def recompute(payload: dict, protocol_path: Path) -> dict:
    ...
    if payload["schema"] != "ingot-public-validation-result-v7":
    ...
    if len(records) != 4 or len(expected_units) != 4:
        raise ValueError("v7 must contain 4 unique evaluation units")
    if any(len(record["episodes"]) != 100 for record in records):
        raise ValueError("every v7 evaluation unit must contain 100 episodes")
    if any(
        len({tuple(episode["initial_run_ids"]) for episode in record["episodes"]})
        != 100
        for record in records
    ):
        raise ValueError("v7 initial designs are not unique within a unit")
...
def main() -> None:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=Path(__file__).parents[1] / "protocol-v7.json",
    )
