#!/usr/bin/env python3
"""Independently recompute the frozen v4 result from episode records."""
...
def recompute(payload: dict, protocol_path: Path) -> dict:
    ...
    if payload["schema"] != "ingot-public-validation-result-v4":
    ...
    if len(records) != 25 or len(expected_units) != 25:
        raise ValueError("v4 must contain 25 unique evaluation units")
    if any(len(record["episodes"]) != 50 for record in records):
        raise ValueError("every v4 evaluation unit must contain 50 episodes")
    if any(
        len({tuple(episode["initial_run_ids"]) for episode in record["episodes"]})
        != 50
        for record in records
    ):
        raise ValueError("v4 initial designs are not unique within a unit")
...
def main() -> None:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=Path(__file__).parents[1] / "protocol-v4.json",
    )
