#!/usr/bin/env python3
"""Run the protocol-frozen v6 holdout public-engineering evaluation."""
...
PROTOCOL_PATH = ROOT / "protocol-v6.json"
PRIMARY = "ingot-v8-with-preregistered-mechanism-features"
ABLATION = "ingot-v8-without-mechanism-features"
DATASETS = {
    "lnp3": {
        "id_field": "setting_id",
        "control_fields": (
        ...
        "context_fields": ("solid_lipid",),
        "outcome_field": "particle_diameter",
        "other_fields": ("drug_loading", "encapsulation_efficiency"),
    },
}
...
def evaluation_fingerprint(protocol: dict) -> str:
    ...
    paths = [
        ROOT / "benchmark_v6.py",
        ROOT / "data" / "lnp3-formulations.csv",
...
def require_frozen(protocol: dict) -> None:
    ...
    if protocol.get("status") != "frozen" or any(
        ...
        raise RuntimeError(
            "protocol-v6 is not frozen; commit the candidate, data, adapter, and draft protocol first"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "protocol-v6 fingerprint does not match the frozen algorithm, adapter, data, dependencies, and protocol"
        )
    ...
    *,
    ...
    while len(episodes) < episode_count:
        if candidate_index > 1_000_000:
            raise RuntimeError(f"unable to generate enough v6 episodes for {unit_id}")
...
def main() -> None:
    ...
    if args.integrity_only:
        ...
        payload = {
            "schema": "ingot-public-validation-result-v6",
            "protocol_sha256": sha256(PROTOCOL_PATH),
