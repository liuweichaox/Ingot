#!/usr/bin/env python3
"""Run the protocol-frozen v7 OER-composition public evaluation."""
...
PROTOCOL_PATH = ROOT / "protocol-v7.json"
PRIMARY = "ingot-v10-with-preregistered-composition-features"
ABLATION = "ingot-v10-without-composition-features"
DATASETS = {
    "oer_plate_3496": {
        "id_field": "setting_id",
        "control_fields": (
...
def require_frozen(protocol: dict) -> None:
    ...
    if protocol.get("status") != "frozen" or any(
        ...
        raise RuntimeError(
            "protocol-v7 is not frozen; commit the candidate, data, adapter, and draft protocol first"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "protocol-v7 fingerprint does not match the frozen algorithm, adapter, data, dependencies, and protocol"
        )
...
def load_rows(dataset: str, protocol: dict) -> list[dict[str, str]]:
    ...
    if not np.allclose(controls.sum(axis=1), 1.0, rtol=0.0, atol=1e-8):
        raise ValueError(f"{dataset} compositions must sum to one")
    if not np.allclose(controls * 10.0, np.round(controls * 10.0), atol=1e-8):
        raise ValueError(f"{dataset} compositions must remain on the 10 at% grid")
    if np.any((controls > 1e-8).sum(axis=1) > 4):
        raise ValueError(f"{dataset} contains a quinary or senary composition")
    ...
    *,
    ...
    while len(episodes) < episode_count:
        if candidate_index > 1_000_000:
            raise RuntimeError(f"unable to generate enough v7 episodes for {unit_id}")
...
def main() -> None:
    ...
    if args.integrity_only:
        ...
        payload = {
            "schema": "ingot-public-validation-result-v7",
            "protocol_sha256": sha256(PROTOCOL_PATH),
