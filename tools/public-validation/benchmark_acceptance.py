#!/usr/bin/env python3
"""Run the protocol-frozen fresh-data optimizer acceptance."""
...
PROTOCOL_PATH = ROOT / "acceptance-protocol.json"
PRIMARY = "ingot-current-policy"
DATASETS = {
    "alkox": {
        "id_field": "setting_id",
        "control_fields": (
        ...
        "context_fields": (),
        "outcome_field": "conversion",
        "other_fields": ("replicate_count", "replicate_sample_std"),
    },
    "p3ht": {
        "id_field": "setting_id",
        "control_fields": (
...
def load_protocol() -> dict:
    ...
    if protocol.get("schema") != "ingot-optimizer-acceptance-protocol":
        raise ValueError("unsupported public validation protocol")
    if set(protocol["sources"]) != set(DATASETS):
        raise ValueError("protocol sources and acceptance adapters disagree")
    expected = {PRIMARY, *protocol["methods"]["baselines"]}
    if len(expected) != 5:
        raise ValueError("acceptance requires one Ingot policy and four baselines")
    unsuccessful = int(protocol["statistics"]["unsuccessful_trial_value"])
    for name, settings in protocol["datasets"].items():
        if name not in DATASETS:
            raise ValueError(f"unsupported acceptance dataset: {name}")
        if tuple(settings["controls"]) != DATASETS[name]["control_fields"]:
            raise ValueError(f"acceptance control order changed for {name}")
        if tuple(settings["contexts"]) != DATASETS[name]["context_fields"]:
            raise ValueError(f"acceptance context order changed for {name}")
        initial = int(settings["initial_observations"])
        additional = int(settings["additional_trial_budget"])
        if initial < 3 or additional < 1:
            raise ValueError(f"invalid acceptance episode budget for {name}")
        if additional + 1 != unsuccessful:
            raise ValueError("all acceptance units must share the capped failure value")
        rule = settings["objective"]["threshold_rule"]
        if (
            rule.get("kind") != "empirical-quantile-within-evaluation-unit"
            or rule.get("method") != "linear"
        ):
            raise ValueError("acceptance requires the frozen within-unit quantile rule")
        if not 0.5 < float(rule["quantile"]) < 1.0:
            raise ValueError("maximization target quantile must lie between one half and one")
    return protocol
...
def evaluation_fingerprint(protocol: dict) -> str:
    ...
    paths = [
        ROOT / "benchmark_acceptance.py",
        ROOT / "prepare_acceptance_fixtures.py",
        *[
            ROOT / protocol["sources"][name]["fixture"]
            for name in sorted(DATASETS)
        ],
        *[
            ROOT / protocol["sources"][name]["source_snapshot"]
            for name in sorted(DATASETS)
        ],
...
def require_frozen(protocol: dict) -> None:
    ...
    if protocol.get("status") != "frozen" or any(
        ...
        raise RuntimeError(
            "acceptance protocol is not frozen; commit the candidate, data, adapter, and draft protocol first"
        )
    ...
    if (
        ...
        raise RuntimeError(
            "acceptance fingerprint does not match the frozen algorithm, adapter, data, dependencies, and protocol"
        )
...
def load_rows(dataset: str, protocol: dict) -> list[dict[str, str]]:
    ...
    source_snapshot = ROOT / source["source_snapshot"]
    if sha256(source_snapshot) != source["source_sha256"]:
        raise ValueError(f"{dataset} source snapshot checksum mismatch")
    ...
    for index, row in enumerate(rows):
        try:
            controls = tuple(float(row[field]) for field in metadata["control_fields"])
            outcomes = tuple(
                float(row[field])
                for field in (metadata["outcome_field"], *metadata["other_fields"])
            )
        except ValueError as error:
            raise ValueError(f"{dataset} row {index} is not numeric") from error
        if not all(math.isfinite(value) for value in (*controls, *outcomes)):
            raise ValueError(f"{dataset} row {index} contains a non-finite value")
        settings.append(
            (*controls, *(row[field] for field in metadata["context_fields"]))
        )
    ...
    replicate_counts = np.asarray([float(row["replicate_count"]) for row in rows])
    replicate_spreads = np.asarray(
        [float(row["replicate_sample_std"]) for row in rows]
    )
    if np.any(replicate_counts < 1) or not np.allclose(
        replicate_counts, np.round(replicate_counts)
    ):
        raise ValueError(f"{dataset} contains an invalid replicate count")
    if np.any(replicate_spreads < 0.0):
        raise ValueError(f"{dataset} contains a negative replicate spread")
    if int(replicate_counts.sum()) != int(source["source_rows"]):
        raise ValueError(f"{dataset} replicate counts do not reconcile to the source")
    ...
    for field in metadata["context_fields"]:
        ...
        expected = {
            str(value)
            for value in protocol["datasets"][dataset]["contexts"][field]
        }
        if actual != expected:
...
def group_evaluation_units(
    ...
    for values, unit_rows in sorted(grouped.items()):
        ...
        suffix = ",".join(f"{name}={value}" for name, value in context.items())
...
def build_campaign(
    ...
    campaign = Campaign(
        f"{unit_id}-fresh-data-acceptance",
        variables,
        [
            Objective(
                objective["name"],
        ...
        context={
            ...
            "protocol": "fresh-data-acceptance",
            **context,
        },
    )
    ...
    *,
    ...
    while len(episodes) < episode_count:
        if candidate_index > 1_000_000:
            raise RuntimeError(
                f"unable to generate enough acceptance episodes for {unit_id}"
            )
...
def paired_effect(records: list[dict], comparator: str, protocol: dict) -> dict:
    ...
    unit_relative_reductions = {
        unit: float((comparison.mean() - primary.mean()) / comparison.mean())
    ...
    response_surface = comparator in {
        "regularized-linear-response-surface",
        "regularized-quadratic-response-surface",
    }
    if response_surface:
        relative_floor = float(statistics["response_surface_noninferiority_margin"])
    else:
        relative_floor = float(
            statistics["model_free_minimum_relative_reduction_ci_lower"]
        )
    unit_floor = float(statistics["evaluation_unit_noninferiority_margin"])
    unit_non_worse = float(
        np.mean(
            [value >= unit_floor for value in unit_relative_reductions.values()]
        )
    )
    success_floor = float(statistics["minimum_success_rate_difference_ci_lower"])
    passed = (
        float(relative_ci[0]) >= relative_floor
        and float(success_ci[0]) >= success_floor
        and unit_non_worse == 1.0
    )
    return {
        ...
        "required_relative_reduction_ci_lower": relative_floor,
        "evaluation_unit_relative_reductions": unit_relative_reductions,
...
def summarize(records: list[dict], protocol: dict) -> dict:
    methods = [PRIMARY, *protocol["methods"]["baselines"]]
    comparisons = [
        paired_effect(records, method, protocol)
        for method in protocol["methods"]["baselines"]
    ]
    response_surfaces = [
        item
    ...
    response_surface_added_value = any(
        item["relative_trial_reduction_ci95"][0] > 0.0
        for item in response_surfaces
    )
    source_groups = {
    ...
    return {
        ...
        "response_surface_added_value": response_surface_added_value,
        "core_experiment_selection": (
            "passed-unseen-data-acceptance"
            if all(item["passed"] for item in comparisons)
            and response_surface_added_value
...
def integrity_report(protocol: dict) -> dict:
    ...
    for name in DATASETS:
        ...
        datasets[name] = {
            "rows": len(rows),
            "source_sha256": protocol["sources"][name]["source_sha256"],
            ...
            "outcome_field_present": metadata["outcome_field"],
...
def main() -> None:
    ...
    if args.integrity_only:
        ...
        payload = {
            "schema": "ingot-optimizer-acceptance-result",
            "protocol_sha256": sha256(PROTOCOL_PATH),
