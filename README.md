...
公开回放已经确认方法在部分物理实验数据上减少追加查询。冻结旧策略在富勒烯数据上不如随机搜索与 maximin，已经停止使用；当前策略修复了该问题，并在同一批已披露数据的 400 次配对开发回归中同时胜过随机、maximin、线性和二次响应面。下一步是在另一批全新数据上验收当前策略。完整数据、失败分项、置信区间和通过规则集中放在[优化器实验效率验证](tools/public-validation/README.md)；真实试点验收见[场景验证](docs/rollout.md)。
