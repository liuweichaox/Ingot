...
450 次冻结配对回放显示，当前策略相对随机搜索减少 49.24%、相对 maximin 减少 62.96%，三个数据集都不劣；但它在 Alkox 和 P3HT 上分别比线性响应面多用 32.08% 和 68.36% 的追加实验，并与二次响应面轨迹一致，因此核心验收未通过。当前实现证明了结构化选点优于盲目探索，但尚未证明方法路由优于选择适用的简单响应面。完整数据、失败分项、置信区间和通过规则见[优化器实验效率验证](tools/public-validation/README.md)；真实试点验收见[场景验证](docs/rollout.md)。
