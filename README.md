...
项目已使用受控的非公开生产历史数据完成导入、运行重建、检验关联和研发观察的内部端到端验证。仓库还提供采用明确许可公开数据的实验效率验证：v2 开发回归覆盖两个制造实验数据集和 10 个上下文；相对固定随机搜索的优效检验通过，但 Crossed Barrel 分项对线性响应面退化并触发数据集护栏，因此总体实验缩减声明为未证明。v3 已加入两个未参与开发的物理实验数据集、四个预先登记基线和机理特征消融，但协议仍处于冻结前草案状态，尚无结果。正式真实历史无泄漏回放和前瞻验证尚未完成，因此不声称任意工厂都能减少实验或研发周期。
...
完整 v3 评估只能在算法和协议提交冻结后运行；草案状态下评估器会拒绝生成结果。
