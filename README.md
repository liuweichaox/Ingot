...
公开回放已经确认当前策略在部分物理实验数据上减少追加查询；最新未见数据验收通过了线性和二次响应面护栏，但在富勒烯数据上不如随机搜索与 maximin，因此核心结论仍未通过。完整数据、失败分项、置信区间和通过规则集中放在[优化器实验效率验证](tools/public-validation/README.md)；真实试点验收见[场景验证](docs/rollout.md)。
