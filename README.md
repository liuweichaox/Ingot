...
**Ingot Chat** 是工程师使用 Ingot 的主要入口。它运行在 Platform Web 中，只读取已有生产事实：日常模式用于查事实和数据完整性；深入调查模式让工艺、质量与反证三个角色审查同一批已验证证据，给出待工程师确认的候选解释。
...
│   ├── Ingot.Platform.Infrastructure/ 中心事实、检测、Webhook 与 Chat 事实工具
