...
| 中心数据存储 | 保存生产事件、检测记录、对象、上下文、关联 ID，并支持查询和 SSE |
| Ingot Chat | 自然语言问题、页面上下文、流式回答、历史记录、限制条件和证据回链 |
| Chat 辅助分析 | 检查数据完整性、追踪生产周期事件链，并返回可回溯证据 |
...
使用方拥有数据源协议与运行方式；Ingot 负责统一事件契约、访问控制、查询和证据回链。详细说明见[宏观架构](docs/architecture.md)与[生产事件规范](docs/rfc-production-events.md)。
