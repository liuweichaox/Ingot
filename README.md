...
<div align="center">
  ...
  <h3>可信生产事实与对话式问题查询平台</h3>

  <p>
    用标准事件接入不同数据源，用 Ingot Chat 查询事实、检查数据质量并回链证据。
  </p>
...
Ingot 为制造现场保存可查询、可验证、可追溯的生产事实。设备、仪器、MES、ERP 或自定义系统由使用方自行接入：将各自的数据转换为标准 `ProductionEvent` 或 `InspectionRecord`，再调用公开 HTTP API。Ingot 不内置设备协议，也不替代现场控制、排产、库存或质量处置系统。

**Ingot Chat** 是唯一面向用户的 AI 入口。它运行在 Central Web 中，只读取已有生产事实，回答问题时展示工具活动、限制条件和可解析证据引用。
