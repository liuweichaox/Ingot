...
项目当前以**光学镜片精密模压 + 三菱 FX3U PLC**为首个真实场景。Edge 采集实际配方与周期曲线，Platform 将周期和检验结果组成可追溯实验样本，周期诊断定位偏差来源与候选变量，Python 优化服务使用高斯过程和受约束贝叶斯优化推荐下一次运行参数。换工艺时，核心闭环不变，只替换变量、结果、安全约束、数据映射和领域特征。
