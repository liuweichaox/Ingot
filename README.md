...
| 阶段 | 解决的问题 |
|---|---|
| 工艺配置 | 平台应理解哪些变量、单位、目标、质量规则和安全边界？ |
...
仓库已实现现场接入、生产运行、上下文、检验、研发实验、诊断候选和数值建议的主要代码路径，并有自动化测试。理解当前成熟度时，需要区分四种证据：
...
项目已使用受控的非公开生产历史数据完成导入、运行重建、检验关联和研发观察的内部端到端验证。仓库还提供采用明确许可公开制造数据的固定离线基准；当前基准已经通过流程、分类上下文隔离和声明边界检查，但严格的“减少实验次数”结论仍为未证明。正式真实历史无泄漏回放和前瞻验证尚未完成，因此不声称已经减少某个比例的实验或研发周期。
...
该基准验证可复现的软件和方法链路，不替代真实项目的影子验证或受控在线验证。当前结果与判定规则见[公开数据离线验证](tools/public-validation/README.md)。
...
- [公开数据离线验证](tools/public-validation/README.md)
