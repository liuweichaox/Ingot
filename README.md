...
项目已使用受控的非公开生产历史数据完成导入、运行重建、检验关联和研发观察的内部端到端验证。仓库还提供采用明确许可公开数据的实验效率验证：v2 开发回归在两个数据集、14 个上下文中证明相对固定随机搜索减少追加实验，并与线性响应面总体非劣；v3 已加入两个未参与开发的物理实验数据集、四个预先登记基线和机理特征消融，但协议仍处于冻结前草案状态，尚无结果。现有结论只适用于固定公开历史池；正式真实历史无泄漏回放和前瞻验证尚未完成，因此不声称任意工厂都能获得相同比例的实验或研发周期缩减。
