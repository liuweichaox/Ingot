...
打开 `http://127.0.0.1:3001`，使用 `demo / demo` 登录。工作台会给出四步入口：打开超差运行 → 核对已复核质量结果 → 与合格运行比较 → 查看候选原因、混杂因素和实验边界。这个演示证明页面与证据工作流可以运行，不证明算法已经找到根因或能为工厂减少实验。
...
仓库已实现现场接入、生产运行、上下文、检验、研发实验、诊断候选和数值建议的主要代码路径，并有自动化测试。评估当前成熟度时，需要区分四种证据：
...
项目已使用受控的非公开生产历史数据完成导入、运行重建、检验关联和研发观察的内部端到端验证。仓库还提供采用明确许可公开数据的实验效率验证：v2 开发回归覆盖两个制造实验数据集和 10 个上下文；相对固定随机搜索的优效检验通过，但 Crossed Barrel 分项对线性响应面退化并触发数据集护栏。v3 在协议冻结后完成两个未参与开发的物理实验数据集、400 个 episode、四个预先登记基线和机理特征消融：相对随机搜索与 maximin 通过，但未优于线性和二次响应面，因此总体实验缩减声明仍为未证明；机理特征的配对消融通过。正式真实历史无泄漏回放和前瞻验证尚未完成，因此不声称任意工厂都能减少实验或研发周期。

- **公开仓库提供**：代码、协议、Schema、合成样例、明确许可且带校验和的公开数据基准、测试、验收方法和结论边界；
- **不进入公开仓库**：真实生产数据、项目标识、工艺参数、质量分布和派生结果；
- **参考部署边界**：默认 Compose 是单 API、单 Worker、单 PostgreSQL 的参考拓扑，不包含 PostgreSQL HA、持续 WAL/PITR、对象存储或受控设备写入。

历史回放、影子验证和受控在线验证分别形成独立证据，某一阶段通过不代表其他阶段通过。详细实现状态见[文档首页](docs/index.md)，验证方法见[场景验证](docs/rollout.md)，生产边界见[生产架构](docs/production-architecture.md)。
