...
**Ingot Chat** 是工程师使用 Ingot 的主要入口。它运行在 Platform Web 中，只读取已有生产数据：日常模式用于查数据和完整性；深入调查模式让工艺、质量与反证三个角色审查同一批已验证证据，给出待工程师确认的候选解释。
...
| Chat 数据工具 | `check_data_quality` 与 `get_cycle_trace` |
...
默认 Compose 启动 PostgreSQL、Platform API 与 Platform Web；Chat 与 Connector Host 均按需启用。`INGOT_OPERATOR_TOKEN` 用于检测事实提交，Chat 使用独立的 `INGOT_CHAT_OPERATOR_TOKEN`。
...
当适配程序位于现场网络、需要本地 SQLite outbox 或无法直连 Platform 时，使用方可启用 **Ingot.Edge.ConnectorHost** 作为可选的本地事件入口。它接收 `ProductionEvent[]`、持久化并以至少一次语义上报 Platform；该路径由使用方选择、部署和运维。
...
Chat 使用受控的只读数据工具，返回结论、限制条件与原始数据引用。它不修改事件、检测记录、配置或设备状态。
