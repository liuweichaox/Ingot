...
感谢你为 Ingot 提交代码、测试、文档或设计建议。所有变更都必须保持生产数据可信、Ingot Chat 只读、数据接入契约稳定和中英文文档一致。

## 开发原则

- `Ingot.Domain`、`Ingot.Edge.Application` 和 `Ingot.Agent` 不依赖具体数据库、模型供应商或设备协议。
- 设备和系统差异由用户的接入程序或现有系统处理，不把厂商 SDK 加入平台核心。
- Chat 模型负责问题理解和回答组织；生产记录查询、数据校验、权限、运行上限和结果与原始记录核对使用确定性代码。
- Chat 数据工具保持只读。
...
- 每个关键数字和结论必须能解析到真实 `RelatedRecordRef`。
- `/api/v1/chat/*` 只接受只读生产记录查询。
- Chat 的回答过程、历史记录和权限必须按用户隔离。

### 连接器

- 连接器统一输出 `ProductionEvent[]`，不得把源协议模型泄漏到核心契约。
- 数据源接入程序由用户实现和部署，并通过标准事件契约向 Ingot 提交生产数据。
- 接入程序必须通过公开事件契约将记录提交到 Ingot，并且不得把源协议模型泄漏到核心契约。
- 接入文档必须说明鉴权、幂等、时间戳、单位、质量字段和可恢复错误。

### API 与存储

- API 输入必须在控制器边界完成类型和权限校验。
- PostgreSQL 用于中心数据存储；SQLite WAL 用于现场事件、outbox 和 Chat 运行。
- 数据库改动必须包含初始化或迁移、并发处理规则、失败处理和集成测试。
...
- 官网只描述已经实现的能力，示例数据必须明确标识。
- 修改公开能力、配置、接口或术语时，同步更新 README、中英文 `docs/`、官网和文档站。
- 页面、接口说明和对外文档遵循[产品用语](docs/product-language.md)，使用现场人员容易理解且含义一致的名称。
