...
Real production data, project and equipment identities, process parameters, quality distributions, sequential traces, and derived results do not enter the public repository. The repository also bundles no public validation datasets, historical protocols, or results; it retains only general contracts, synthetic tests, and this validation method. Deployers manage evaluation data, baselines, budgets, decision rules, and results in their controlled environment.

Whether a deployer publishes an evaluation conclusion is governed by its data policy and business requirements; the Ingot repository does not aggregate or endorse quantitative benefit claims for a particular scenario.
