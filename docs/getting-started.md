...
| 目标 | 使用路径 | 完成标志 |
|---|---|---|
| 评估产品流程 | [五分钟合成演示](#五分钟合成演示) | 完成超差运行比较并查看候选原因 |
...
工作台可进入运行追溯、检验记录和工艺追因，支持选择同类运行、比较实际条件与轨迹，并查看候选原因。配方推荐和工程师决策已有后端接口，当前前端尚未提供对应工作流页面。所有数据均为合成数据；该演示验证页面和流程，不证明真实工艺收益。

常用操作：

- 使用 `Ctrl+K`（macOS 为 `⌘K`）打开功能搜索，用上下方向键选择、`Enter` 打开、`Esc` 关闭；搜索结果按当前岗位显示。
- 分析助手中，`Enter` 发送、`Shift+Enter` 换行；中文输入法选词期间的 `Enter` 不发送。
- 操作通知按触发顺序逐条显示，每条显示 3.5 秒，也可点击关闭。填写表单时，批次一致性、校准到期和部分分析规则的关键说明直接显示在字段下方。
