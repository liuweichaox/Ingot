...
Real production data and derived results remain inside the confidentiality boundary and are not submitted or disclosed for public reproduction. Current frozen public acceptance uses three previously unseen physical-experiment datasets and four strong baselines. It passes against random search and maximin and matches the quadratic surface, but materially trails the linear surface on Alkox and P3HT, so core method routing fails. This is not factory-benefit evidence; public wording must identify the datasets, comparators, validation stage, and failed subgroups.
