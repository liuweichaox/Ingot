...
如需本地 SQLite outbox，使用方可部署 `Ingot.Edge.ConnectorHost` 并向其提交事件；它是可选的本地入口，适配实现与运行由使用方负责。
...
Platform 按 `eventId` 和 `(edgeId, seq)` 识别重复。调用方仍应保持本地确认状态，避免把未确认和新事件混在同一无序批次中。
