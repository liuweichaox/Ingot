...
The current reach-specification path first uses the GP posterior to enforce outcome-safety probability thresholds, then admits the simplest applicable response model from visible observations. A regularized linear surface is used when a raw control clears both Pearson linear-association and Spearman monotonic-rank thresholds. Without declared mechanism features, the policy uses a raw-control quadratic surface. With mechanism features but no more observations than first-order joint-model coefficients, leave-one-out error selects between raw linear and joint raw-plus-mechanism linear surfaces. Only after observations exceed that minimum complexity gate does the policy use an equal-rank ensemble of mechanism-only and joint quadratic surfaces. Method routing reads revealed observations, never candidate outcomes. The GP continues to supply objective predictions, intervals, and safety probabilities. The hypothesis-validation path instead prioritizes outcome uncertainty and identifiability in the key variables. Thresholds, regularization values, and the model version are implementation choices that require fresh replay and frozen validation, not product principles.
