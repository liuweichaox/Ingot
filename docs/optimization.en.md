...
The current method-selection policy first repaired known response-surface regressions in 100 paired development episodes on disclosed OER data. A subsequent 400-episode unseen-data acceptance selected Buckminsterfullerene flow reactions and Suzuki coupling before freezing the protocol and reading outcomes. The policy reduces additional experiments by 16.74% versus linear response surfaces and 7.25% versus quadratic response surfaces, passing both. On Fullerenes, however, it is −23.01% versus random search and −40.65% versus maximin, triggering both dataset guardrails, so core acceptance fails. The preregistered process features improve trials by only 0.20%, with a 95% CI lower bound of zero; they also fail and remain disabled.

This failure shows that routing now avoids regression when a response surface is applicable, but lacks an evidence threshold for falling back to maximin when neither response surface fits and coverage is more effective. The complete frozen result remains in the validation directory. A successor may use it to develop a model-insufficiency fallback, but a new public decision still requires another unseen dataset group.
