...
In 100 paired development episodes on disclosed OER data, the current method-selection policy reduces additional experiments by 53.66%, 54.42%, 50.52%, and 7.00% versus random search, maximin, linear response surfaces, and quadratic response surfaces, respectively; every dataset guardrail passes. The paired ablation for generic composition features is −0.10%, with no stable benefit, so the current policy rejects their admission. This result shows that known response-surface regressions are repaired on development data; it does not pass unseen-data acceptance. The next public decision must freeze the algorithm, data selection, objective, budget, and gates before outcomes are read.
