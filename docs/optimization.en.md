...
The current reach-specification path first uses the GP posterior to enforce outcome-safety probability thresholds, then routes methods from visible observations. Histories with fewer than two observations per raw control remain on a regularized linear surface. With enough data, strong monotonic evidence retains the linear surface; otherwise the quadratic surface is used. On low-dimensional problems, disagreement between the two surfaces' best candidates combines response ranking with maximin coverage, preventing a weak early model from exploiting one local region. Euclidean fallback is disabled above three dimensions, where distance concentration makes it unreliable. GP posterior specification probability joins at 25% weight only after at least six visible observations per raw control. Declared mechanism features must pass both capacity and predictive-gain gates; failure removes them from both the surrogate and selection rule. Routing reads revealed observations and candidate controls only, never candidate outcomes or dataset names. The GP continues to supply objective predictions, intervals, and safety probabilities. The hypothesis-validation path instead prioritizes outcome uncertainty and identifiability in the key variables. Thresholds, regularization values, and the model version are implementation choices that require fresh replay and frozen validation, not product principles.
...
A frozen predecessor completed 400 unseen-data episodes on Buckminsterfullerene flow reactions and Suzuki coupling after data selection and protocol freeze. It reduced additional experiments by 16.74% versus linear and 7.25% versus quadratic response surfaces, but Fullerenes was −23.01% versus random and −40.65% versus maximin. It therefore failed core acceptance and has been retired.

That failure exposed the absence of a coverage fallback when model evidence is weak. The current policy adds exploration routing based on sample capacity, response-surface agreement, and dimension. Across 400 paired development replays on those now-disclosed data, it reduces experiments by 48.90%, 21.37%, 39.78%, and 32.91% versus random, maximin, linear, and quadratic response surfaces; all four gates and both dataset guardrails pass. This establishes that the known problem is repaired, not independent effect evidence, so the next public decision still requires another fresh dataset group. Process-feature ablation is −0.14%; features remain disabled while core selection continues on raw controls.
