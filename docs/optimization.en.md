...
The current reach-specification path first uses the GP posterior to enforce outcome-safety probability thresholds, then routes methods from visible observations. A raw control that clears both Pearson linear-association and Spearman monotonic-rank thresholds directly admits the regularized linear surface. Otherwise, an equal-rank consensus of raw linear and quadratic surfaces is the simple model. GP posterior specification probability joins that consensus only after at least three visible observations per raw control. Declared mechanism features must pass both capacity and predictive-gain gates: each candidate response model needs at least three observations per coefficient, and leave-one-out error must improve by at least 10% over the raw quadratic surface. Failure removes derived features from both the surrogate and selection rule. Routing reads revealed observations only, never candidate outcomes or dataset names. The GP continues to supply objective predictions, intervals, and safety probabilities. The hypothesis-validation path instead prioritizes outcome uncertainty and identifiability in the key variables. Thresholds, regularization values, and the model version are implementation choices that require fresh replay and frozen validation, not product principles.
...
That result is the v2 development regression. v3 uses NASA wind-tunnel airfoil-noise and Delft yacht-hydrodynamics physical experiments as external data not used during v2 development. It preregisters random search, maximin space filling, linear response surface, and quadratic response surface, and performs a paired ablation between the same optimizer with and without mechanism-derived features. Across 400 post-freeze episodes, the reduction is 75.10% versus random search (95% CI 72.67%–77.32%) and 9.22% versus maximin (95% CI 0.33%–17.19%), both passing. It is -13.70% versus the linear response surface (95% CI -22.94%–-4.55%) and -5.06% versus the quadratic response surface (95% CI -17.07%–6.51%), both failing, so the overall claim remains not demonstrated. Mechanism features reduce trials by 22.64% versus the no-mechanism variant (95% CI 17.63%–27.50%), passing the paired ablation. These results establish only additional-result-query efficiency in the fixed public physical-experiment pools, not development-cycle reduction for an arbitrary factory.

Successor v6 fails its quadratic-response-surface and feature-ablation gates in three LNP3 formulation contexts. v7 then freezes v10 on four measured OER composition plates not used for development. It reduces trials by 57.70%, 58.01%, and 53.67% versus random, maximin, and linear response surfaces, respectively, passing all three. The aggregate reduction versus the quadratic response surface is 11.67% (95% CI 4.96%–17.81%), but only 2/4 plates are non-worse, so that gate fails. Composition-feature ablation is 0.13% (95% CI −2.00%–2.23%), also with only 2/4 plates non-worse. The overall claim remains not demonstrated, and the latest failure keeps the reach-specification path fail-closed and downgraded; aggregate advantage cannot override a per-plate failure.
