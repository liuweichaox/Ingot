...
> 文档状态：**目标设计与实施基线**。本文定义 Ingot 从研发验证环境进入长期生产运行必须满足的故障模型、数据语义、安全边界和验收门槛；该定义不表示仓库自带的单机 Compose 已经达到高可用。

本文定义生产拓扑、故障模型、数据语义和安全准入要求，不作为安装教程。默认单机部署仅适用于开发验证和受控试点；长期工厂运行还须验证故障条件下的数据持久性、站点权限隔离、安全规则执行和备份恢复能力。
...
时序数据库可以保存原始测点、过程帧和聚合，但不能成为审批、运行状态或执行回执的唯一记录源。时序存储不可用时允许暂停曲线查询和新分析，不能转而在浏览器、Agent 或 Optimizer 内形成平行正式状态。
...
达到软水位时先限制交互式大查询和后台重算；达到硬水位时停止接受新的非关键分析任务。生产采集不得通过无日志丢弃来“保护”平台。Edge 本地容量不足时执行预先配置的现场策略，并产生不可抹除的丢弃审计与告警。
...
| 固定证据 | 被报告、审批或正式结论引用的输入 | 独立保留策略和法律保留，不随普通时序策略删除 |
...
| 范围 | 当前仓库事实 | 仍需部署方或后续阶段完成 |
|---|---|---|
| P0 生产契约 | 已实现 `SiteId`、规范事件信封、未知 Schema 失败关闭、配置版本、质量标记、内容哈希、站点范围和数据库约束；`.env.example` 已声明 RPO/RTO、积压、新鲜度、容量余量和观察周期，验收脚本要求实测值与证据标识 | 正式证据的分层固定/删除策略仍需按现场保留规则完成；仓库脚本不能替代真实现场验收 |
| P1 可恢复单元 | 已提供一致性逻辑备份/恢复、校验和、监控 Compose profile、基础看板/告警以及有限的 API/Worker/Optimizer 故障演练 | 默认 Compose 仍是单 API、单 Worker、单 PostgreSQL；没有入口负载均衡、PostgreSQL HA、持续 WAL/PITR、异机不可变备份或对象存储 |
| P2 数据平面 | 已有 Edge outbox、幂等摄入、确定性拒绝隔离、后台租约和死信基础 | 迟到水位线、完整重算/重放运维面、热温冷生命周期和公平配额尚未形成生产闭环 |
| P3 受控行动 | 当前产品保持观测、分析和建议边界，Agent 分析工具只读，Optimizer 不批准或执行动作 | 动作账本、签名调度、Edge Safety Executor、设备回读和逐级准入尚未实现，不得声称设备闭环控制 |

下列 P0–P3 是阶段完成定义。表中“已实现”只表示仓库存在代码、测试或部署资产，不表示某个现场已经通过容量、恢复、HA、安全或持续观察验收。
