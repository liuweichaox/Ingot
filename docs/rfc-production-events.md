...
| | |
|---|---|
| 状态 | Draft v0.2（v0.1 基础上：确立源中立抽象、资产/来源分离、项目更名 Ingot） |
| 日期 | 2026-07-16 |
...
| D11 | 立即更名 Ingot | 发布 1.0 后再改；或保持原名 | 立即。名称准确承载“把生产数据熔炼成不可变事实”的产品定位；外部引用尚少、GitHub 自动重定向、当前也是改名成本最低的时刻 | 一次性全仓 PR；历史外链短期失效（重定向缓解） |
...
## 12. 更名方案：DataAcquisition → Ingot

更名是定位声明的一部分：项目名不再描述输入动作（采集），而描述平台对数据完成的价值转换——**把生产数据炼成事实**。

### 12.1 名称含义

**Ingot** 意为“锭”，这个隐喻同时对应产品能力、价值感与制造业语境：

1. **冶炼隐喻**：遥测数据像矿砂——量大、分散、单位价值低；Ingot 把它熔炼为标准化、事件化的生产事实。锭一旦铸成便具有确定形态，可以不可变地保存、堆叠和流通，对应本 RFC 的统一事件模型、不可变事件日志与事件块存储。
2. **价值隐喻**：锭是经过提纯和标准化后形成的价值单位，天然表达“从原始数据中提炼可信价值”。金锭在不同文化中也普遍关联财富与信任，中文语境里的“锭/元宝”还带有积极、稳固的品牌联想。
3. **制造业血统**：锭是铸造、冶金和钢铁生产中的原生词汇，不是借来的科技概念。它让制造业用户无需额外解释，就能理解产品与生产现场、加工过程和工业事实之间的关系。

名称也满足工程使用要求：五个字母、两个音节，读写直接，可自然形成 `Ingot.Edge.Agent`、`Ingot.Domain`、`ingot` CLI 等命名；视觉上可用“锭的堆叠”表达不可变事件块的累积。建议品牌标语为：

> **把生产数据炼成事实。**

英文可对应为：

> **Forge production data into facts.**
...
1. 完成 GitHub organization / repository、NuGet 包前缀、商标与域名核查；主域名优先评估 `ingot.io`、`ingot.dev`
2. GitHub 仓库更名（平台自动重定向）
3. 全仓替换：`.sln`、`.csproj`、命名空间与 `using`、docker-compose 服务名、CI 引用
4. README / docs / 徽章 / 示例路径同步；README 保留一行"曾用名 DataAcquisition"便于搜索与老链接语境
5. 置顶公告 issue，附本 RFC 说明更名、新定位与名称含义
