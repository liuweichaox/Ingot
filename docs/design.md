...
2. **现场接入**：配置总览、现场节点、通信驱动及多来源字段与工艺变量映射；
3. **工艺配置**：工艺数据字典、工艺规范、分析规则、质量、工装及配置发布；
...
一级业务入口在工作台之后按“现场接入 → 工艺配置 → 生产运行 → 质量管理 → 工艺追因 → 工艺研发”排列。现场接入先从配置总览盘点来源和依赖，工艺配置再建立稳定语义、规则与发布版本；随后才进入生产、质量、追因和研发。这是产品导航对完整闭环的概括，不是与上方六个阶段逐项对应：生产运行同时承载生产准备、采集与追溯，质量管理承载检验及质量偏差工作，完整的数据闭环还需要数据可信度和运行上下文等跨入口证据。
