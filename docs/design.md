...
5. **工艺优化**：研发项目、实验验证及其可复用资产；
6. **工艺定义**：工艺数据模型、工艺规范、分析、质量、工装及工艺配置方案；
7. **设备接入**：现场节点、通信驱动、设备点位与工艺变量映射。

一级业务入口在工作台之后按“工艺定义 → 设备接入 → 生产运行 → 质量管理 → 工艺追因 → 工艺优化”排列，用简化的依赖顺序说明为什么必须先建立可信数据，再进入追因和优化。这是产品导航对完整闭环的概括，不是与上方六个阶段逐项对应：生产运行同时承载生产准备、采集与追溯，质量管理承载检验及质量偏差工作，完整的数据闭环还需要数据可信度和运行上下文等跨入口证据。
