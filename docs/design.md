...
当前 Web 信息架构按六个面向工程任务的产品域组织：

1. **工作台**：运行、质量、数据可信度和下一步行动；
2. **生产运行**：真实运行、生产事件、周期对比、生产切换与工装资产；
3. **质量管理**：检验任务、质量追因、检测定义、质量方案和复核；
4. **工艺研发**：问题、候选原因、假设、实验、结果和工艺窗口；
5. **数据与配置**：工业对象、数据质量、场景配置、变量模型、配方、现场节点和设备接入；
6. **系统管理**：用户、岗位权限、平台状态和运行日志。

生产运行和数据与配置的二级导航按任务分组，避免把追溯、现场上下文、工装资产、工艺配置和设备接入平铺在一个无层级列表中。
