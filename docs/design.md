...
Ingot 的核心价值由[品牌规范](brand.md)固定：把每次真实运行变成可比较、可验证的工程证据，帮助工艺工程师减少无效实验，更快找到达到目标的工艺条件。
...
前四步按业务依赖把现场活动整理为可信运行事实；后两步使用这些事实帮助工程师形成和验证决策。工艺追因解释已经发生的结果；工艺研发通过可证伪实验验证候选，并在目标与安全边界内优化下一步实验。受约束优化是工艺研发中的一种能力，不是自动控制或独立业务入口。两者必须读取同一份证据。
...
7. **工艺研发**：研发项目、实验设计、受约束优化、实验验证及研发成果。

一级业务入口在工作台之后按“现场接入 → 工艺配置 → 生产运行 → 质量管理 → 工艺追因 → 工艺研发”排列。这个导航顺序优先照顾岗位高频任务，不等于上方的业务依赖顺序：新场景仍先定义和发布工艺语义，再把真实来源映射到该语义。生产运行同时承载生产准备、采集与追溯，质量管理承载检验及质量偏差工作，完整的数据闭环还需要数据可信度和运行上下文等跨入口证据。
...
一个成功参数点只能成为候选工艺设置。工艺操作域需要独立确认、重复性、边界或交互验证以及明确适用范围。
