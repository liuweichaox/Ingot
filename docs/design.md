...
当前 Web 信息架构按七个显式产品域组织：

1. **全局总览**：运行、质量、数据可信度和下一步行动；
2. **生产周期（Cycles）**：真实运行、生产事件和周期对比；
3. **生产上下文（Manufacturing）**：生产切换、配方、材料、组件和工装状态；
4. **质量检验（Inspections）**：检验任务、结果、定义、质量方案和复核；
5. **工艺研发**：问题、候选原因、假设、实验、结果和工艺窗口；
6. **数据与接入**：工业对象、变量模型、场景配置、现场节点和数据质量；
7. **身份与系统（Identity）**：用户、岗位权限、平台状态和运行日志。
