...
2. **工艺定义**：工艺数据模型、工艺规范、分析、质量、工装及工艺配置方案；
3. **设备接入**：现场节点、通信驱动、设备点位与工艺变量映射；
4. **生产运行**：运行记录、运行事件、对象目录、生产准备与工装装卸；
5. **质量管理**：检测录入、独立复核、质量记录和质量偏差分析；质量岗位可直接进入日常任务；
6. **工艺追因**：追因工作台、运行对比、数据可信度和分析助手；AI 是分析手段，不单独成为业务域；
7. **工艺研发**：研发项目、实验验证及其研发资产。

一级业务入口在工作台之后按“工艺定义 → 设备接入 → 生产运行 → 质量管理 → 工艺追因 → 工艺研发”排列，用简化的依赖顺序说明为什么必须先建立可信数据，再进入追因和研发。这是产品导航对完整闭环的概括，不是与上方六个阶段逐项对应：生产运行同时承载生产准备、采集与追溯，质量管理承载检验及质量偏差工作，完整的数据闭环还需要数据可信度和运行上下文等跨入口证据。

系统管理通过独立入口承载用户、岗位权限、平台状态、运行日志和助手评测，不与业务任务竞争一级导航。各业务域的二级导航优先显示日常高频任务，再显示准备或维护动作；所有现有 URL 与数据契约保持稳定，导航只改变工程师发现功能的路径。
...
Platform 内部按用例分离策略与实现：`Platform.Application` 保存可脱离数据库测试的应用规则和存储端口，`Platform.Infrastructure` 实现 PostgreSQL 事务、外部服务和跨上下文适配。工艺研发、采集配置、制造上下文、事件、身份、洞察、分析与检验的数据库无关端口，以及其中的多步规则和用例，均归属 Application；PostgreSQL Store、优化器 HTTP 客户端、后台宿主和证据装配器归属 Infrastructure，并通过按模块的组合入口注册。API 只处理传输、鉴权，并通过 Application 边界委托业务操作；首用户引导由 Migrator 在事务锁内完成，周期维护由 Worker 执行。并发幂等与原子写入继续由数据库事务和约束保证，不上提为内存规则。工艺研究规则不能直接读取检验模块；检验、过程运行和配置证据只能由显式登记的装配适配器（当前为 `ResearchObservationAssembler`）转换为研究观察。
