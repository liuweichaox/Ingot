...
3. **工艺配置**：配置总览、数据字典、配方版本、分析规则、质量配置、工装配置及配置发布；
...
Agent 使用两条彼此分离的证据路径。运行、检验、配置和优化等结构化生产事实始终由授权业务工具查询；SOP、手册、报告和工程记录等文档知识，只从所属项目、站点、产品与设备范围内已复核的来源和片段中检索。文档检索在 PostgreSQL 中组合全文/相似词匹配与可选语义向量排序，返回片段级来源、页码或 Sheet、来源 SHA 和内容哈希。语义嵌入未启用、不可用或查询失败时自动退回关键词检索；检索结果只为回答提供只读证据，不能替代当前生产数据查询、证明根因或修改设备。
...
- 计划配方版本与设备实际应用的配方版本分别保留；同类比较按分析方案声明的实际配方版本 cohort 执行，不得把计划版本当成设备实际版本；
