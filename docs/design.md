...
2. **生产运行**：工业对象、真实运行、运行事件、生产准备与工装装卸；
3. **质量管理**：检测录入、独立复核、质量记录和质量偏差分析；质量岗位可直接进入日常任务；
4. **工艺分析**：分析助手、数据可信度和运行对比；AI 是分析手段，不单独成为业务域；
5. **工艺研发**：研发项目、实验验证及其可复用资产；
6. **工艺定义**：工艺数据模型、工艺规范、分析、质量、工装及工艺配置方案；
7. **设备接入**：现场节点、通信驱动、设备点位与工艺变量映射。
