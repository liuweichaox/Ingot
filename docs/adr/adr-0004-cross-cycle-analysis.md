...
| **返回载荷**（进入模型上下文的字节） | 同样是那 500 行 | **严格有界的统计摘要** |

现在这两件事被同一个 `Limit` 绑在一起，这才是真正的设计缺陷。把 `Limit` 从 500 提到 500000 会同时放开两者 —— 计算范围对了，返回载荷灾难：

- 10 个周期 × 每周期数千条事件 = 数十万 token，成本与延迟不可接受；
- 更致命的是，**这等于让模型自己从原始行里做算术**。而 `IEvidenceVerifier` 存在的全部意义，就是防止答案里的数字来自模型而不是来自确定性计算。给模型灌全量明细，是从架构内部拆掉自己最值钱的那道墙。
...
var stats = await events.GetScopeStatsAsync(context.ActorId, scope, ct);
// 明细窗口：周期配对 / 序号连续性 / 上下文缺失需要逐行数据，仍限 500 行
var rows = await events.QueryAsync(context.ActorId, scope with { Limit = 500 }, ct);
```
...
### 原则二：`Data` 有界给模型，`Artifacts` 全量给人

`AnalysisToolResult` 当前只有一个 `Data`，直接进模型上下文。需要拆成三层：
...
public sealed record AnalysisToolResult
{
    ...
    public IReadOnlyList<AnalysisArtifact> Artifacts { get; init; } = [];  // 给人：全量结果、图表、CSV
    public IReadOnlyList<EvidenceRef> Evidence { get; init; } = [];        // 指向全量的可验证链接
...
{
    public required string ArtifactId { get; init; }   // UUIDv7
    public required string Kind { get; init; }         // table | series | chart-spec | csv
    public required string Label { get; init; }
    public required string Url { get; init; }          // 前端拉取，不进模型上下文
    public required long RowCount { get; init; }
}
```

**模型看摘要，人看全量，证据链接指向全量。** 这样既不牺牲分析深度，也不牺牲数字可信度 —— 用户在 Web 上能点开完整的 8000 行对比明细和曲线，模型手里始终只有"P95 = 58.3，n = 1240"这种已经算好的、可被重算验证的数字。
...
ADR-0003 §4.3 的产品族 / 工艺路线本体，让"同类"可以是推导出来的（同产品族、同工艺路线、同模具），而不只是字符串相等。**先做 3.1，语义层在其上生长**。
...
返回：匹配的周期摘要列表 + **判定为同类的依据**（哪些键相等、哪些不等）。

把"找同类"独立成工具，是为了让"凭什么说它们可比"这件事**可见、可质疑、可修正**。这正是 `BoundedInvestigationWorkflow` 里 challenge 角色应该攻击的地方。藏在对比工具内部的隐式匹配逻辑，是不可审计的。
...
- 检测结论对比（合格率、关键特性偏移）
- `Artifacts`：完整的逐周期明细表 + 可下载 CSV

**设计要点**：默认返回统计量，不返回逐周期明细。明细放 `Artifacts`。

### 4.3 `get_cycle_trace` 的兼容扩展

保持单周期语义不变（它是"看明细"的工具，500 行上限合理）。跨周期由上面两个新工具承担。**不要把 `get_cycle_trace` 改成支持数组** —— 那会让一个工具同时承担"看明细"和"做对比"两种语义，输入 schema 和返回形状都会退化。
...
3. **时序基础模型编码**（ADR-0003 §3.3）：把整段序列编码成向量 / 特征再进对话。**原始波形本来就不该进 LLM 上下文** —— 让通用对话模型逐点读时序数据，既贵又不可靠。
...
这些护栏和现有的 `MaxToolCalls`、轮数上限属于同一类设计 —— **有界性是这个平台的特性，不是限制**。放开行数上限的同时，必须补上等价的资源边界，否则只是把不可控从上下文长度转移到了数据库负载。
...
| **4** | `AnalysisToolResult` 拆出 `Artifacts` + `Data` 字节上限 | 契约变更，越早越好；同时立即约束现有工具 |
| **5** | `find_comparable_cycles` + `compare_cycles` | 用户可感知的能力交付 |
| **6** | `AgentGuards` 扫描预算与周期数上限 | 必须与 5 同期上线，不能延后 |
| **7** | 异步分析作业 Surface | 承接需要原始序列的重分析 |
...
| `Data` / `Artifacts` 分离 | 模型上下文可控且不做算术 | 契约破坏性变更；前端要新增 artifact 拉取与渲染 |
...
- **异步作业成为主要路径时**：是否需要独立的计算服务，而不是跑在 Central API 进程内。

---

参见 [ADR-0001](adr-0001-local-database-and-fact-store.md)、[ADR-0002](adr-0002-manual-inspection-input.md)、[ADR-0003](adr-0003-capability-amplification-roadmap.md)、[生产事件规范](../rfc-production-events.md)。
