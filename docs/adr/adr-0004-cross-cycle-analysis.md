...
`CheckDataQualityTool` 使用内部分页读取完整范围，再返回有界摘要：

```csharp
// 全范围聚合：快速获得总条数与新鲜度
var stats = await events.GetScopeStatsAsync(context.UserId, scope, ct);
// 完整明细：内部按页读取，分页大小不构成分析范围上限
var rows = await events.QueryAllAsync(context.UserId, scope, ct);
```
...
必须逐行、且无法用 SQL 表达的检查也要通过流式或分页方式覆盖完整范围，不能用固定窗口代替完整性结论。
...
保持单周期含义不变，但读取该周期的完整事件链；500 只允许作为内部传输分页大小，不能成为周期明细上限。跨周期仍由上面两个工具承担。**不要把 `get_cycle_trace` 改成支持数组** —— 那会让一个工具同时承担"看明细"和"做对比"两种含义，输入 schema 和返回形状都会退化。
...
- **大范围转异步**：数据量较大时转为异步作业继续处理完整范围，不通过固定条数上限或静默抽样改变分析语义
- **物化优先**：能命中 `cycles` / `cycle_features` 的查询绝不下沉到明细表
- **超时降级**：超时返回部分结果 + 明确的 limitation，而不是失败或静默截断

这些护栏约束执行方式和模型载荷，不约束参与计算的数据范围。平台可以分页、流式或转异步，但最终结论必须覆盖用户选择的完整范围。
