...
真实生产数据及其派生结果始终留在受控环境。上述完整报告是有权限、可审计的内部证据工件，不以公开原始数据、项目标识、参数分布或项目结果为完成条件。公开仓库只提供通用协议、Schema、合成样例和合规测试。
...
3. **预注册历史回放**：同时完成工程问题回放和生产等价逐次回放，在受控内部报告中保留全部失败与限制；公开材料只说明协议和结论边界。
