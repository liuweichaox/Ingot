...
首个长期验证场景已经建立数据接入与追因流程，历史回放、影子与受控在线验证仍在推进；具体行业与设备信息不进入公开仓库，这个场景本身不是产品边界。只有第二个明显不同的真实制造场景无需修改核心证据、实验和行动契约，通用性才得到支持。
...
真实生产数据及其派生结果始终留在受控环境。上述完整报告是有权限、可审计的内部证据工件，不以公开原始数据、项目标识、参数分布或项目结果为完成条件。公开仓库提供通用协议、Schema、合成样例、明确许可且带校验和的公开数据基准和合规测试；公开基准验证可复现的软件与方法行为，不代替真实项目报告。
...
当前代码检查点不能等同于验证通过：可信身份、事件完整性、质量准入、应用层用例、历史回放/影子/在线报告服务、机理知识门禁和生产验收工具已经进入仓库；正式无泄漏真实历史报告、新项目影子结果、受控在线结果、第二场景和外部协议实现仍未完成。因此前两批主要从“补齐代码路径”转为“用真实负载和故障演练证明这些路径”，科学线的下一硬门槛仍是第 3 批预注册历史回放。

| 优先级 | 工作 | 完成定义 |
|---|---|---|
| P0 | 运行身份、实际值、上下文、检验关联和质量有效性 | 每条分析记录可以唯一回到真实运行和有效结果 |
| P0 | 历史证据冻结、重放、事务和恢复 | 输入不可被事后改写，失败恢复不破坏来源链 |
| P0 科学线 | 历史问题与生产等价逐次回放 | 形成受控内部预注册、无泄漏、带基线和失败项的审核报告 |
| P0 科学线 | 冻结公开外部数据评估 | 未参与开发的数据、强基线和机理消融按冻结协议运行，不利结果完整保留 |
...
1. **可信身份与质量链**：核心代码路径和数据库约束已建立；继续用真实站点隔离、时钟异常、缺失实际值和错误质检关联做对抗验收。
2. **历史证据装置**：冻结、内容哈希、事务、回放服务和恢复工具已建立；继续完成现场保留规则、重算一致性和恢复演练证据。
3. **冻结公开外部数据评估**：先提交算法和协议，再运行未参与开发的物理实验数据、强基线和机理特征消融；不得根据首次结果修改冻结条件。
4. **预注册历史回放**：同时完成工程问题回放和生产等价逐次回放，在受控内部报告中保留全部失败与限制；该真实项目的公开材料只说明协议和结论边界，另行发布的公开数据基准不得冒充真实项目结果。
5. **工艺决策案例**：固定数据质量、基线、首次偏离、候选、反证、混杂、缺失和实验提案结构。
6. **Agent 评测样本**：黄金问题、运行快照和评测存储已建立；继续扩充工程师问题、正确拒绝、引用覆盖、权限和工具调用的对抗样本。
7. **前瞻影子验证**：冻结版本化变量、映射、上下文、约束、策略和工程师拒绝原因。
8. **协议化只读与提议能力**：从内部 API 提炼稳定领域工具，并提供 MCP、OpenAPI 或 SDK 适配。
9. **受控行动准备**：动作账本、授权令牌、策略检查、设备确认、停止与回滚。
10. **第二场景与规范候选**：验证跨场景核心语义，发布 Schema、验证器和参考实现候选版。
