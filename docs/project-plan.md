...
3. **预注册历史回放**：同时完成工程问题回放和生产等价逐次回放，在受控内部报告中保留全部失败与限制；该真实项目的公开材料只说明协议和结论边界，另行发布的公开数据基准不得冒充真实项目结果。
4. **工艺决策案例**：固定数据质量、基线、首次偏离、候选、反证、混杂、缺失和实验提案结构。
5. **Agent 评测飞轮**：黄金问题、运行快照和评测存储已建立；继续扩充工程师问题、正确拒绝、引用覆盖、权限和工具调用的对抗样本。
