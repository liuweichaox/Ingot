...
Ingot 的名称与标识共享同一个隐喻：**数据源适配提交的生产事件、检测事实与生产上下文是矿砂，Ingot 把它们熔炼成标准化、可信、可堆叠的生产事实。**
...
- **产品类别**：可信生产事实与工艺调查平台 / Trusted Production Facts & Process Investigation Platform
- **主要用户**：制造现场的生产、工艺、质量、设备与 IT 角色，尤其是需要快速独立部署的中小制造企业
- **产品架构**：Ingot 是平台品牌；Ingot Chat 是工程师的主要入口；深入调查是 Chat 中由工艺、质量和反证角色共同审查证据的高级模式
- **价值链**：使用方数据源适配 → 标准生产事件 + 检测事实 + 生产上下文 → 可信生产事实 → Ingot Chat 日常问答或深入调查 → 工程师确认
