...
Ingot 的名称与标识共享同一个隐喻：**真实周期、检验结果和工艺知识是矿砂，Ingot 把它们熔炼成两样东西——对这一炉的解释，和对下一炉的建议。**
...
> **Ingot — 看清这一炉，算准下一炉。** Explain this run, optimize the next.

## 品牌定位

- **产品类别**：开源工艺追因与优化系统 / Open-source Process Diagnosis & Optimization
- **一句话定义**：Ingot 是面向高成本、小样本制造实验的开源工艺追因与优化系统，把真实周期、过程轨迹和检验结果连成可追溯证据，解释这一炉的偏差来源，并推荐下一炉的参数
- **两个核心问题**：这一炉为什么没达到规格，偏差来自哪个变量、哪段轨迹；下一炉应该怎样设置，才能用尽可能少的实验达到规格
...
| Tagline | 看清这一炉，算准下一炉。/ Explain this run, optimize the next. |
...
- 术语统一：品类句、tagline 和 metadata 里的能力名一律写「追因」；「周期诊断」「偏差归因」可以作为具体功能名继续使用，但不用来替代品类词。英文对外写 *diagnosis*，*root-cause attribution* 只在技术文档中作解释性说明。
