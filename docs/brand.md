...
Ingot 的名称与标识共享同一个隐喻：**数据源适配提交的生产事件、检测记录与生产关联信息是矿砂，Ingot 把它们熔炼成标准化、可信、可追溯的生产数据。**
...
- 两块**钢色底锭**：已沉淀的事件日志——append-only，越垒越高的历史记录
- 顶部**金锭**：刚刚铸成的最新记录；锭面上的一道反光是出炉时的余温
...
> **Ingot — 把分散数据整理成完整生产履历。** Turn scattered plant data into a complete production history.

## 品牌定位

- **产品类别**：制造数据采集与工艺分析平台 / Manufacturing Data Collection & Process Analysis Platform
- **主要用户**：制造现场的生产、工艺、质量、设备与 IT 角色，尤其是需要快速独立部署的中小制造企业
- **产品架构**：Ingot 是平台品牌；Ingot Chat 是工程师的主要入口；快速查询用于查看生产记录，综合分析用于比较参数并列出可能原因
- **价值链**：接入设备与业务系统 → 汇集生产过程和检测结果 → 形成完整生产履历 → 查询异常或综合分析 → 工程师确认
- **系统边界**：MES 可以是可选数据源或集成目标，但不是运行前提；Ingot 不以排程、订单执行、库存或考勤定义自身

对外表达优先说明“汇集生产过程、关联检测结果、比较参数变化、分析可能原因”，不通过贬低其他工业数据平台或声称其缺少某项能力来建立差异。
...
| 色名 | 值 | 用途 |
|---|---|---|
| Molten Gold | `#FFC94A → #E8891A` | 金锭（最新记录）、强调色 |
