...
1. 导出数据源 CSV，获得连接实例的表头和已有设备行；
2. 增加或修改同类设备的 Edge、身份、地址与密钥引用，再导入数据源 CSV；
3. 导出任务绑定 CSV，获得模板—数据源组合的表头和已有绑定；
4. 为新增数据源选择模板版本，再导入任务绑定 CSV。
...
- 发布探查必须至少观察到配置引用的每个值、质量、上下文、设备时间、设备序号和配方版本路径；运行时可选点位也不例外。设备不提供源序号时应留空，而不是保留一个无法验证的默认路径。间歇字段应在设备能产生该字段时完成探查。
