...
- Tooling installation: the effective interval of an assembly revision on a machine. A machine can have only one active installation.
...
- Without MES: operators enter setup once when changing product, recipe, or tooling; subsequent cycles inherit it automatically.
