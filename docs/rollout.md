...
[发展规划](project-plan.md)定义为什么建设、长期成为谁以及何时晋级；本文件只定义历史回放、影子验证和受控在线实验怎样预注册、执行、否证、内部审核并形成对外结论边界。三条验证线各自回答不同问题，不能互相替代。
...
其中“序贯建议不优于适用基线”已经落实为可执行门禁：所有达到规格类优化都必须引用当前策略与机理快照下、经独立复核且通过的历史回放，当前优化器模型版本也必须实际出现在回放轨迹中。缺失、失败或版本不一致时请求直接停止，并返回响应面或传统 DOE 的降级路径；不能只在报告中记录失败后继续发建议。
