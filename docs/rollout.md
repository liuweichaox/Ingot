...
真实生产数据、项目与设备标识、工艺参数、质量分布、逐次轨迹和派生结果不进入公开仓库或公开报告。公开材料可以发布通用协议、Schema、合成样例、明确许可且带校验和的公开数据基准、验收方法和不泄露项目事实的结论边界。合成数据验证契约和软件行为；公开数据还可以复现方法比较，但两者都不能冒充真实项目证据或证明工厂收益。
