...
该 Compose 配置为 `operator` 启用完整事实范围。生产部署应按角色和网络边界配置实际所需的数据范围，并将所有机密注入 Secret Store 或受保护环境变量。
...
- 事件类型语义不兼容时提升 `eventTypeVersion` 或新增事件类型；
- 模型或提示版本升级前应通过 Chat 评测，并保留运行、工具和证据审计记录。

## 运行边界

Ingot 提供事实存储、查询和对话。设备协议、现场安全、网络隔离、凭据轮换、适配程序可用性和设备控制由部署方及现场系统负责。
