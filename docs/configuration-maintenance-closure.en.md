...
Maintenance behavior follows record type:
...
- Shop-floor records (tooling installations, production contexts, inspection records, attachments, and reviews) are append-only. The platform provides history, details, interval closure, review, and audit actions without overwriting records that already occurred.
...
Page acceptance must verify that a newly created item can be found, opened, maintained according to its record type, and retained by historical references.
...
Normal pages do not display instructional alert banners. Titles, fields, grouping, state, and empty states explain the page purpose. Successful operations use transient feedback; persistent alerts are reserved for errors, blockers, or risks that require user action.
