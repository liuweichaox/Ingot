...
- 生成受控的结构化查询计划；
- 调用 `check_data_quality` 检查事件完整性、生产信息缺失、事件新鲜度和可用范围；
- 调用 `get_cycle_trace` 按 `correlationId` 返回排序后的周期事件链；
- 流式展示计划、只读工具活动、回答、注意事项和相关记录；
- 保存当前用户 可见的对话历史，并支持取消和 SSE 续读。
...
生产 Compose 默认关闭 Chat。启用时同时配置模型、模型密钥、用户令牌和数据范围：
...
Platform Web 与 HTTP API 使用用户 `operator` 和 `INGOT_CHAT_OPERATOR_TOKEN`。生产部署应按每个用户 所需数据范围配置访问权限。
