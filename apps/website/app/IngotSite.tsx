...
const copy = {
  zh: {
    ...
    optimizerText: "系统先判断真实运行是否完整、可比并覆盖至少两种实际配方，再按样本数量、数据覆盖和安全约束选择响应面或受约束优化。因果确认和越界探索才使用独立的受控验证。",
    methodA: "确认数据可用",
    ...
    openText: "Ingot 采用 Apache-2.0 许可，可在厂内自托管。现场采集、运行证据、工艺追因、真实运行优化、可选受控验证和知识沉淀位于同一仓库；公开验证协议与结果可以独立复现。",
  ...
  en: {
    ...
    titleB: "Improve the next recipe.",
    ...
    openText: "Ingot is Apache-2.0 licensed and self-hostable inside the plant. Field acquisition, run evidence, process diagnosis, real-run optimization, optional controlled validation, and knowledge preservation live in one repository; public validation protocols and results are independently reproducible.",
    command: "git clone https://github.com/liuweichaox/Ingot.git\ncd Ingot\ncp .env.example .env\ndocker compose -f docker-compose.app.yml up -d --build",
    ...
    footer: "Ingot · Understand this run. Improve the next recipe.",
  },
} as const;
