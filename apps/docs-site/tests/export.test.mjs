...
test("exports the bilingual product documentation journey", async () => {
  ...
  for (const slug of ["getting-started", "design", "optimization", "mechanism-knowledge", "data-connection", "project-plan", "rollout", "deployment", "faq", "brand", "open-source-dependencies"])
...
test("publishes the data-supported process R&D journey and public references without interface documentation", async () => {
  ...
  assert.equal(search.length, 24);
  assert.deepEqual(
    [...new Set(search.map((item) => item.slug))].sort(),
    ["", "brand", "data-connection", "deployment", "design", "faq", "getting-started", "mechanism-knowledge", "open-source-dependencies", "optimization", "project-plan", "rollout"],
  );
