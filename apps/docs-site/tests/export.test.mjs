...
test("exports the bilingual product documentation journey", async () => {
  ...
  for (const slug of ["getting-started", "design", "optimization", "data-connection", "rollout", "deployment", "faq"])
...
test("publishes the AI process R&D product journey without interface or developer documentation", async () => {
  ...
  assert.equal(search.length, 16);
  assert.deepEqual(
    [...new Set(search.map((item) => item.slug))].sort(),
    ["", "data-connection", "deployment", "design", "faq", "getting-started", "optimization", "rollout"],
  );
