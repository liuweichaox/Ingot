...
Thank you for contributing code, tests, documentation, or design feedback. Every change must preserve trusted production records, read-only Ingot Chat, stable ingestion contracts, and synchronized Chinese and English documentation.
...
- Chat models handle question interpretation and response composition. Deterministic code owns record retrieval, data validation, authorization, run limits, and related records verification.
- Chat record tools remain read-only.
...
- Every key number and conclusion must resolve to a real `RelatedRecordRef`.
- `/api/v1/chat/*` accepts only read-only record queries.
- Chat runs, histories, events, and permissions remain isolated by user.

### Connectors

- Connectors emit normalized `ProductionEvent[]` and never leak source protocol models into core contracts.
- Users implement and deploy source adapters, which submit production records through the standard event contract.
- Adapters submit records through the published event contract and never leak source-protocol models into core contracts.
...
- PostgreSQL stores platform records. SQLite WAL stores shop-floor events, outbox records, and Chat runs.
- Database changes include initialization or migration, concurrency handling rules, failure handling, and integration tests.
...
- The product website describes implemented capabilities only and labels sample records explicitly.
...
- UI copy, API descriptions, and public documentation follow the [product language guide](docs/product-language.en.md) and use names that plant users understand consistently.
