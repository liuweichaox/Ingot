...
- 先按 GP 结果安全概率过滤候选，再按可见观察执行线性/二次响应面方法准入；
...
NumPy/SciPy GP 只保留为冷启动与回归基线；三个有效观察后，生产路径进入 BoTorch 内核。达到规格的候选选择不会默认强迫 GP 胜过简单方法：某个原始控制量同时达到 Pearson 与 Spearman 证据门槛时使用正则化线性响应面；没有声明式机理特征时使用原始控制量二次响应面；存在机理特征但可见观察尚未超过一阶联合模型的系数数目时，以留一法在原始线性与“原始控制量 + 机理特征”联合线性响应面之间选择。观察超过这一最低复杂度门槛后，才使用“仅机理特征 + 联合特征”的二次响应面等权秩集成。该门控只读取已经揭示的观察，不读取候选结果。GP 继续负责预测区间与结果安全概率。
