...
候选点只在历史运行覆盖的区域内生成：每个变量不得超出其观察区间加余量（观察跨度的 10% 与变量量程的 2% 中的较大者），候选点相对观察中心的马氏距离也不得超过观察点自身最大值再放宽 10%。平台会用同一批运行独立复算该包络，两者不一致或建议越出包络时停止建议。包络内候选不足时，服务返回 422 并说明生产运行尚未覆盖足够的参数空间。该包络只约束 `reach-specification`；`validate-hypothesis` 在安全边界内探索可辨识信息，不施加也不返回观察覆盖包络。
