...
NumPy/SciPy GP 只保留为冷启动与回归基线；三个有效观察后，生产路径进入 BoTorch 内核。达到规格的候选选择不会默认强迫 GP 胜过简单方法：某个原始控制量同时达到 Pearson 与 Spearman 证据门槛时使用正则化线性响应面；否则，有声明式机理特征时使用“仅机理特征 + 原始控制量与机理特征联合”的二次响应面等权秩集成，没有机理特征时使用原始控制量二次响应面。GP 继续负责预测区间与结果安全概率。
