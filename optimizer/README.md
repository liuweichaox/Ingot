...
本目录实现 Ingot 方法工具箱中的代理模型和序贯配方优化能力。它根据平台提供的研发项目快照与有效观察，返回下一批**待工程师审核**的候选配方参数；它不保存业务状态，也不直接控制设备。GP/BO 是适合昂贵小样本序贯运行的当前方法，不是所有工艺问题的唯一答案，也不重新定义产品核心价值。
...
- 决策意图仅为 `reach-specification`：在观察覆盖包络内逼近规格；
- 由项目版本化配置声明的安全派生特征，不根据行业、设备或变量名称触发隐藏逻辑；
- 安全基线局部冷启动、待执行配方点和幂等批次；
- 只能从真实历史配方池中选择的离线回放（`POST /v1/historical-replay`）；
- 无状态 `POST /v1/suggestions` HTTP 契约；
- 合成数字孪生回放，用于验证算法契约。

NumPy/SciPy GP 只保留为冷启动与回归基线；三个有效观察后，生产路径进入 BoTorch 内核。达到规格时，正则化线性响应面是默认方法。样本达到最低容量后，系统对线性面与二次面做配对留一预测，并比较归一化的目标排序误差；只有二次面的改善连续三个扩展历史都超过一个标准误才升级，证据不明时保留线性面。只有非线性证据已经成立且可见观察达到每个原始控制量至少六条，GP 后验达标概率才接管选点。声明式机理特征也必须满足容量和配对预测证据；未通过时从代理模型中移除。所有准入只读取已经揭示的观察和候选参数，不读取候选结果，也不按数据集名称分支。GP 始终负责预测区间和结果安全概率。
...
仓库不内置公开或现场验证数据、历史协议和结果文件。`optimizer/tests` 只验证算法契约、约束、确定性、失败关闭和回放不读取未来结果；具体场景的效果评估由使用者在仓库外使用自己的数据完成。
...
响应包含推荐参数、每项目标的均值与 95% 区间、预计距规格距离、可行概率、采集值、模型版本、推荐理由，以及本次实际施加的观察覆盖包络 `coverage_envelope`。

候选点只在历史运行覆盖的区域内生成：每个变量不得超出其观察区间加余量（观察跨度的 10% 与变量量程的 2% 中的较大者），候选点相对观察中心的马氏距离也不得超过观察点自身最大值再放宽 10%。平台会用同一批运行独立复算该包络，两者不一致或建议越出包络时停止建议。包络内候选不足时，服务返回 422 并说明生产运行尚未覆盖足够的参数空间。推荐仅使用 `reach-specification`，并始终落在观察覆盖包络内。
...
历史池回放只回答“在已经做过的配方里，模型能否更早排到达标点”。每一行必须提供有限数值 `occurred_at`，并按时间严格递增；缺失、重复或乱序会被拒绝，服务不会自动排序，以免掩盖未来数据泄漏。它不为未做过的配方伪造结果，也不能单独证明上线后一定节省多少运行次数。重复配方应先按既定统计方法聚合。
...
回放返回字段已作破坏性更新：汇总使用 `median_runs_to_specification` 和 `mean_runs_to_specification`，历史池回放使用 `original_order_runs_to_specification`，单次优化器回放使用 `total_run_count` 和 `additional_run_count`。读取这些响应的自建消费者须随服务同步升级；本仓库不提供旧字段兼容层。

这次响应契约变更对应 Optimizer HTTP API `0.6.0`（Python 包 `0.5.0`）。
...
1. `ExecutionKey` 直接映射现场运行标识，平台自动把过程特征、控制参数实值和检验结果组成每次真实运行的观察；
2. 平台将项目定义、有效观察和约束完整发送给本服务；
3. 本服务无状态计算下一批参数；
4. 平台创建带输入哈希、模型版本和预测区间的下一配方建议；
5. 工程师作出决定后，平台关联后续真实运行并冻结质量结果；若这些运行用于验证某个假设，结果置信区间会更新假设的支持、否决或不确定结论。
