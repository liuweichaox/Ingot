...
NumPy/SciPy GP 只保留为冷启动与回归基线；三个有效观察后，生产路径进入 BoTorch 内核。达到规格时，某个原始控制量同时达到 Pearson 与 Spearman 强单调门槛就直接使用正则化线性响应面；否则以线性、二次响应面的等权秩共识作为简单基线。只有可见观察达到每个原始控制量至少三条，GP 后验达标概率才与响应面秩等权融合。声明式机理特征还要通过更严格的双门槛：每个候选响应模型至少具有“系数数目三倍”的可见观察，且留一误差相对原始二次响应面改善至少 10%；未通过时，派生特征同时从代理模型和选点规则中移除。所有准入只读取已经揭示的观察，不读取候选结果，也不按数据集名称分支。GP 始终负责预测区间和结果安全概率；这些实现门槛仍须由冻结回放验证，而不是产品承诺。
