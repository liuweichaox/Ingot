...
NumPy/SciPy GP 只保留为冷启动与回归基线；三个有效观察后，生产路径进入 BoTorch 内核。达到规格时，样本不足每个原始控制量两条就先使用正则化线性响应面；样本足够后，强单调证据继续使用线性面，否则使用二次面。低维问题中，如果线性面与二次面对最佳候选意见不一致，响应面排序会与 maximin 空间覆盖排序组合，避免在模型证据不足时过早困在局部区域；四维以上不使用欧氏距离回退。只有可见观察达到每个原始控制量至少六条，GP 后验达标概率才以 25% 权重加入。声明式机理特征还要通过更严格的容量与留一预测增益门槛；未通过时，派生特征同时从代理模型和选点规则中移除。所有准入只读取已经揭示的观察和候选参数，不读取候选结果，也不按数据集名称分支。GP 始终负责预测区间和结果安全概率；这些实现门槛仍须由冻结回放验证，而不是产品承诺。
