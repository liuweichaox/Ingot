...
The NumPy/SciPy GP remains a cold-start and regression baseline. Online suggestions, historical replay, and synthetic replay all use one engine-selection entry point: fewer than three valid observations use the sequential cold start and may apply NumPy GP priors; three or more use BoTorch. For specification seeking, candidate selection does not force the GP to outrank an applicable simpler model. A regularized linear response surface is admitted when a raw control clears both Pearson and Spearman evidence thresholds. Without declared mechanism features, the policy uses a raw-control quadratic surface. With mechanism features but no more visible observations than first-order joint-model coefficients, leave-one-out error selects between raw linear and joint raw-plus-mechanism linear surfaces. Only after observations exceed that minimum complexity gate does the policy use an equal-rank ensemble of mechanism-only and joint quadratic surfaces. The gate reads revealed observations, never candidate outcomes. The GP still supplies prediction intervals and outcome-safety probabilities. Every caller relies on the selected engine's `suggest` path to enforce measured outcome-safety constraints and must not instantiate a concrete engine directly.
