...
The NumPy/SciPy GP remains a cold-start and regression baseline. Online suggestions, historical replay, and synthetic replay all use one engine-selection entry point: fewer than three valid observations use the sequential cold start and may apply NumPy GP priors; three or more use BoTorch. For specification seeking, a raw control that clears both Pearson and Spearman strong-monotonicity thresholds directly admits the regularized linear surface. Otherwise, an equal-rank consensus of linear and quadratic response surfaces is the simple baseline. GP posterior specification probability joins that rank consensus only after at least three visible observations per raw control. Declared mechanism features face a stricter two-part gate: each candidate response model needs at least three observations per coefficient, and its leave-one-out error must improve by at least 10% over the raw quadratic surface. Failure removes derived features from both the surrogate and the selection rule. Admission reads revealed observations only, never candidate outcomes or dataset names. The GP always supplies prediction intervals and outcome-safety probabilities; the frozen replay protocol, not product wording, must validate these implementation thresholds. Every caller relies on the selected engine's `suggest` path to enforce measured outcome-safety constraints and must not instantiate a concrete engine directly.
