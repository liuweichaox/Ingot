...
///     Central 通过 EventId 与 (EdgeId, Seq) 去重。
/// </summary>
public sealed class HttpEventShipper : IEventShipper
{
    ...
    public async Task RunAsync(CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(_options.CentralApiBaseUrl))
            throw new InvalidOperationException("启用事件上行时必须配置 Edge:CentralApiBaseUrl。");
        ...
        http.BaseAddress = new Uri(_options.CentralApiBaseUrl.TrimEnd('/') + "/");
        ...
        _logger.LogInformation(
            "事件上行已启动：EdgeId={EdgeId}, Central={Central}, BatchSize={BatchSize}",
            edgeId,
            http.BaseAddress,
            batchSize);
