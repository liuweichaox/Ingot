...
/// </summary>
public sealed class PostgresAgentRunStore(NpgsqlDataSource dataSource)
    : IAgentRunStore, IAgentRunImportStore
{
        ...
        CancellationToken ct = default)
    {
        ...
        await using var create = new NpgsqlCommand(
            ...
            transaction);
        BindRun(create, run);
        var created = await create.ExecuteNonQueryAsync(ct).ConfigureAwait(false) == 1;
        if (created)
        {
            foreach (var item in events.OrderBy(static value => value.Sequence))
            {
                await using var append = new NpgsqlCommand(
                    ...
                    connection,
                    transaction);
                append.Parameters.AddWithValue(run.RunId);
                append.Parameters.AddWithValue(item.Type);
                append.Parameters.AddWithValue(item.OccurredAt);
                append.Parameters.Add(new NpgsqlParameter
                {
                    NpgsqlDbType = NpgsqlDbType.Jsonb,
                    Value = item.Data?.GetRawText() ?? (object)DBNull.Value
                });
                await append.ExecuteNonQueryAsync(ct).ConfigureAwait(false);
            }
        }
        await using (var backfill = new NpgsqlCommand(
                         """
                         UPDATE golden_question_evaluations
                         SET agent_run_snapshot = $2, agent_run_snapshot_hash = $3
                         WHERE agent_run_id = $1 AND agent_run_snapshot IS NULL
                         """,
                         connection,
                         transaction))
        {
            backfill.Parameters.AddWithValue(run.RunId);
            backfill.Parameters.Add(new NpgsqlParameter
            {
                NpgsqlDbType = NpgsqlDbType.Jsonb,
                Value = JsonSerializer.Serialize(run, JsonOptions)
            });
            backfill.Parameters.AddWithValue(GoldenQuestionEvaluator.SnapshotHash(run));
            await backfill.ExecuteNonQueryAsync(ct).ConfigureAwait(false);
        }
        await transaction.CommitAsync(ct).ConfigureAwait(false);
        return created;
    }
