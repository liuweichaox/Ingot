    ...
    IInspectionMasterDataStore masterData) : IInspectionWorkflowService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var completed = completedEvents
            ...
            .OrderByDescending(static item => item.Event.OccurredAt)
            .ToArray();
        var operationRunIds = completed.Select(static item => item.Event.CorrelationId!).ToArray();
        ...
        var tasks = completed
            ...
            .Select(item => BuildTask(
                ...
                latestReviews))
            .Where(task => string.IsNullOrWhiteSpace(normalizedStatus) ||
