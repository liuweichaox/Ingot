...
public sealed class PostgresCycleScientificComputeEngine : IAsyncDisposable
{
    ...
    public PostgresCycleScientificComputeEngine(IConfiguration configuration)
    {
    ...
    public async Task<WholeCycleAnalysisResult> ComputeAndVerifyAsync(
        string correlationId,
        DateTimeOffset cycleStartedAt,
        DateTimeOffset cycleCompletedAt,
        WholeCycleAnalysisResult reference,
        CancellationToken ct = default)
    {
        ...
        var threshold = reference.Quality.MedianIntervalMs is > 0
            ...
            : double.MaxValue;
        var signals = new List<CycleSignalStatistic>(reference.Signals.Count);
        foreach (var signal in reference.Signals)
        {
            var refined = new List<CycleSignalFeature>(signal.Features.Count);
    ...
    private async Task<IReadOnlyList<CyclePhaseSummary>> LoadPhasesAsync(
        string correlationId,
        DateTimeOffset completedAt,
        IReadOnlyList<CyclePhaseSummary> reference,
        CancellationToken ct)
    {
        ...
        var result = new List<CyclePhaseSummary>(database.Count);
