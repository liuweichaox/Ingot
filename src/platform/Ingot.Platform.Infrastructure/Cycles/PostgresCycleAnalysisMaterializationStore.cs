...
public sealed class PostgresCycleAnalysisMaterializationStore : ICycleAnalysisMaterializationStore, IAsyncDisposable
{
    ...
    public PostgresCycleAnalysisMaterializationStore(
        ...
        ILogger<PostgresCycleAnalysisMaterializationStore> logger)
    {
        ...
        CancellationToken ct = default)
    {
        ...
        await using var command = _dataSource.CreateCommand(
            ...
            FROM cycle_analysis_materializations
        ...
        if (!await reader.ReadAsync(ct).ConfigureAwait(false))
            return null;
        var result = JsonSerializer.Deserialize<WholeCycleAnalysisResult>(reader.GetString(0), JsonOptions)
                     ?? throw new InvalidOperationException($"周期 {key.CorrelationId} 的物化分析结果无法反序列化。");
    ...
    public async Task<CycleAnalysisBackfillJob> AddBackfillJobAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        await using var command = _dataSource.CreateCommand(
            """
            INSERT INTO cycle_analysis_backfill_jobs(job_id, status, payload, created_at, updated_at)
    ...
    public async Task<CycleAnalysisBackfillJob> SaveBackfillJobAsync(
        CycleAnalysisBackfillJob job,
        CancellationToken ct = default)
    {
        await InitializeAsync(ct).ConfigureAwait(false);
        ...
        if (await command.ExecuteNonQueryAsync(ct).ConfigureAwait(false) == 0)
            throw new KeyNotFoundException("周期分析回填任务不存在。");
        return job;
    }

    public async Task<CycleAnalysisBackfillJob?> GetBackfillJobAsync(
        Guid jobId,
        CancellationToken ct = default)
    {
        await InitializeAsync(ct).ConfigureAwait(false);
        await using var command = _dataSource.CreateCommand(
            "SELECT payload::text FROM cycle_analysis_backfill_jobs WHERE job_id = @job_id;");
        command.Parameters.AddWithValue("job_id", jobId);
        var payload = await command.ExecuteScalarAsync(ct).ConfigureAwait(false) as string;
        return payload is null
            ? null
            : JsonSerializer.Deserialize<CycleAnalysisBackfillJob>(payload, JsonOptions);
    }

    public async Task<IReadOnlyList<CycleAnalysisBackfillJob>> ListBackfillJobsAsync(
        CancellationToken ct = default)
    {
        await InitializeAsync(ct).ConfigureAwait(false);
        await using var command = _dataSource.CreateCommand(
            "SELECT payload::text FROM cycle_analysis_backfill_jobs ORDER BY created_at DESC LIMIT 100;");
        await using var reader = await command.ExecuteReaderAsync(ct).ConfigureAwait(false);
        var jobs = new List<CycleAnalysisBackfillJob>();
        while (await reader.ReadAsync(ct).ConfigureAwait(false))
        {
            var job = JsonSerializer.Deserialize<CycleAnalysisBackfillJob>(reader.GetString(0), JsonOptions);
            if (job is not null)
                jobs.Add(job);
        }
        return jobs;
    ...
    public async Task<IReadOnlyList<CycleFeatureAggregate>> QueryFeatureAggregatesAsync(
        string? signalCode,
        string? phaseCode,
        string? featureCode,
        DateTimeOffset? from,
        ...
        CancellationToken ct = default)
    {
        ...
        await using var command = _dataSource.CreateCommand(
            ...
            SELECT
              ...
              f.phase_code,
              f.feature_code,
              COUNT(DISTINCT f.correlation_id),
              MIN(f.feature_value),
              MAX(f.feature_value),
              AVG(f.feature_value),
            ...
            ORDER BY COUNT(DISTINCT f.correlation_id) DESC,
        ...
        await using var reader = await command.ExecuteReaderAsync(ct).ConfigureAwait(false);
        var rows = new List<CycleFeatureAggregate>();
        while (await reader.ReadAsync(ct).ConfigureAwait(false))
        {
            rows.Add(new CycleFeatureAggregate
            {
                SignalCode = reader.GetString(0),
                PhaseCode = reader.GetString(1),
                FeatureCode = reader.GetString(2),
                CycleCount = reader.GetInt64(3),
    ...
    public async Task<IReadOnlyList<string>> ListDirtyCorrelationIdsAsync(
        int limit,
        CancellationToken ct = default)
    {
        await InitializeAsync(ct).ConfigureAwait(false);
        ...
        await using var command = new NpgsqlCommand(
            ...
            DO UPDATE SET
              ...
              invalidated_at = CASE
                WHEN cycle_analysis_materializations.invalidated_source_max_ingest_id >
                     EXCLUDED.source_max_ingest_id
                  THEN cycle_analysis_materializations.invalidated_at
                ELSE NULL
              END,
              invalidation_reason = CASE
                WHEN cycle_analysis_materializations.invalidated_source_max_ingest_id >
                     EXCLUDED.source_max_ingest_id
                  THEN cycle_analysis_materializations.invalidation_reason
                ELSE NULL
              END,
    ...
    private static async Task DeleteDetailsAsync(
        ...
        CancellationToken ct)
    {
        foreach (var table in new[] { "cycle_features", "cycle_phases" })
        {
            await using var command = new NpgsqlCommand(
                $"""
                 DELETE FROM {table}
    ...
    private static void AddKeyParameters(NpgsqlCommand command, CycleAnalysisMaterializationKey key)
    {
        command.Parameters.AddWithValue("correlation_id", key.CorrelationId);
