    ...
    PlatformUserResolver userResolver) : PlatformApiController
{
        ...
        [FromBody] ResearchRecipeRecommendationDecisionRequest request, CancellationToken ct)
    {
        ...
        return await ExecuteForProjectAsync(recommendation.ProjectId, true, async identity =>
        {
            ...
            if (project?.Status != ResearchProjectStatuses.Active)
                return StateConflict("只有激活中的研发项目才能登记工程师决定。");
            return Ok(await recipeRecommendationDecisions
                .RecordDecisionAsync(recommendationId, recommendationKey, request, identity.UserId, ct).ConfigureAwait(false));
        }, ct);
    }
