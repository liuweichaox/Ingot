...
public sealed record ExecutionBoundary
{
    ...
    /// 运行观察到的结束时刻。
    /// - 若运行未结束，为 null。
    /// - 结束时刻由 process.execution.ended 事件给出，或启发式推断（超时/无事件一段时间）。
    /// </summary>
    ...
    /// <summary>
    /// 运行边界的置信度。
    /// - Complete: 观察到明确的 process.execution.started 和 process.execution.ended。
...
{
    /// <summary>观察到明确的 process.execution.started 和 process.execution.ended 事件。</summary>
    Complete = 0,
