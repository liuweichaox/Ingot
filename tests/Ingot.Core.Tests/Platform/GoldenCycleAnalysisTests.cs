...
/// <summary>
///     「黄金周期」端到端回归：用一条经过工艺确认的光学模压周期喂进产品实际使用的
///     <see cref="WholeCycleAnalysisEngine"/>，断言它对真实数据说出
...
/// </summary>
public sealed class GoldenCycleAnalysisTests
{
    // 周期起点（cycle.started 的 OccurredAt）。真实数据为 2026-06-01 08:00:00。
    private static readonly DateTimeOffset CycleStart = new(2026, 6, 1, 8, 0, 0, TimeSpan.Zero);
    // 周期终点（cycle.completed 的 OccurredAt）= 起点 +95s（08:01:35）。
    private static readonly DateTimeOffset CycleEnd = CycleStart.AddSeconds(95);

    // CYC-0001 的 5 条 process.sample（cycle.started/completed 不是采样，引擎会自动过滤）。
    ...
    [Fact]
    public void RealMoldingCycle_ProducesPhysicallyTrueAndDeterministicAnalysis()
    {
        var rows = BuildRealCycle();
        ...
        var repeated = new WholeCycleAnalysisEngine().Analyze(
            BuildRealCycle(), CycleStart, CycleEnd, MoldingModel(), MoldingPlan("mean", "max", "min"));
    ...
    private static IReadOnlyList<PlatformProductionEvent> BuildRealCycle()
    {
        var events = new List<PlatformProductionEvent>();
        long ingestId = 1;
        foreach (var (offsetSeconds, tUpper, force) in RealSamples)
        {
            var at = CycleStart.AddSeconds(offsetSeconds);
    ...
    private static CycleSignalFeature TempFeature(WholeCycleAnalysisResult result, string code)
        => result.Signals.Single(s => s.Code == "temperature").Features.Single(f => f.Code == code);

    private static CycleSignalFeature ForceFeature(WholeCycleAnalysisResult result, string code)
    ...
    private static ProcessDataModel MoldingModel()
        => new()
        {
            ...
            Acquisition = new AcquisitionModel
            {
                DataItems =
                [
                    new ProcessDataItemDefinition { Code = "temperature", SourceField = "上模温", Nullable = false },
                    new ProcessDataItemDefinition { Code = "force", SourceField = "合模力", Nullable = false }
                ]
            }
        };
